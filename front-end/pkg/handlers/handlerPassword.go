...
// Password is the handler func executed after middleware validation
// it holds a context sent by the middleware
func (rep *Repository) Password(w http.ResponseWriter, r *http.Request) {
	// Check URL
	if !strings.HasPrefix(r.URL.Path, "/profile/password") {
	...
	case 2:
		render.RenderTemplate(w, "admin.profile.password.page.gohtml", td)
	case 3:
		render.RenderTemplate(w, "manager.profile.password.page.gohtml", td)
	case 4:
		render.RenderTemplate(w, "user.profile.password.page.gohtml", td)
	default:
		var empty any
		render.RenderTemplate(w, "public.unauthorized.page.gohtml", empty)
	}
