...
let allEmpEmails = new Map()
window.currentUploadInfo = ""
allEmployees.set(0, 'not defined')
allEmpEmails.set(0, 'not defined')
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    API.getAllEmployees().then(resp => {
        ...
        allEmpEmails = Common.updateEmpEmailList(resp.data, allEmpEmails)
    ...
    // When it shows populate the modal with claim doc
    document.getElementById('uploadedClaimAttachment').addEventListener('show.bs.modal', function (e) {
        API.getUploadedClaimFiles(window.currentUploadInfo).then(resp => {
            console.log(resp.data)
            Helpers.populateUploadedFiles(resp.data, window.currentUploadInfo)
        })  
    })
})
